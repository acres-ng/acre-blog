...
export function ArticleSidebar({ articleUrl, title }: ArticleSidebarProps) {
  return (
    <aside className="flex flex-col gap-6">
      {/* Newsletter card */}
      <div className="bg-acre-green-dark text-white rounded-2xl p-6">
      ...
      {/* Share this post */}
      <div>
        <p className="text-sm font-semibold text-acre-text mb-3">
          Share this post
        </p>
        <div className="flex items-center gap-3">
          {SHARE_LINKS.map((link) => (
            <a
              key={link.name}
              href={link.href(articleUrl, title)}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`Share on ${link.name}`}
              className="text-acre-muted hover:text-acre-green transition-colors"
            >
              {link.icon}
            </a>
          ))}
